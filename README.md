...
这是一个用于查询和管理中国绿色电力证书交易信息的Web应用。应用支持用户注册登录、添加查询网址、自动抓取交易数据，并提供按日期筛选的查询功能。

### ✨ 核心功能

- 🔐 **用户认证**：用户注册、登录、权限管理（第一个用户自动成为管理员）
- 🌐 **网址管理**：添加、查看、删除查询网址
- 🔍 **数据查询**：自动抓取交易数据，支持日期筛选
- 📊 **数据展示**：完整展示10个交易信息字段（项目名称、招标/投标/中标单位、总价、绿证单价、详情链接、通道类型、绿证年份、交易日期）
- 💾 **数据存储**：所有数据自动保存到Supabase数据库
