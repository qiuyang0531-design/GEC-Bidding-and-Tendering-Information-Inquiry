...
6. ✅ 成交量 (quantity) - 数字类型（绿证数量）
7. ✅ 绿证单价 (unit_price) - 数字类型
8. ✅ 具体标的信息链接 (detail_link) - URL
9. ✅ 通道类型 (is_channel) - 布尔值或null（通道/非通道/未标注）
10. ✅ 绿证年份 (cert_year) - 文本类型（支持单年份"2025"或多年份"2024/2025"）
11. ✅ 招标开始日期 (bid_start_date) - 日期格式
12. ✅ 招标结束日期 (bid_end_date) - 日期格式
13. ✅ 中标日期 (award_date) - 日期格式
...
✅ 通道类型支持三种状态（通道、非通道、未标注）
✅ 绿证年份支持多年份格式（单年份"2025"或多年份"2024/2025"）
✅ 日期字段拆分为三个（招标开始日期、招标结束日期、中标日期）
✅ API查询已修复（支持新的日期字段）
✅ 成交量字段已添加（显示在总价和单价之间）
✅ 网址查询状态显示（在网址列表中显示成功/失败状态和错误信息）
✅ 网址编辑功能（可修改网址和备注名称）
✅ 实用的数据抓取实现（HTML表格解析、列表解析、智能字段提取）
✅ 详情链接提取修复（正确处理相对路径，显示在表格最右侧）
...
- **通道类型**：支持三种状态
  - "通道"（is_channel = true）：明确标注为通道交易
  - "非通道"（is_channel = false）：明确标注为非通道交易
  - "-"（is_channel = null）：未标注通道类型（有些项目没有此信息）
- **绿证年份**：支持单年份和多年份
  - 单年份：如"2025"（仅涵盖一个年度）
  - 多年份：如"2024/2025"（涵盖多个年度）
  - 数据库字段类型为TEXT，灵活支持各种格式
- **日期字段**：区分招标和中标信息
  - **招标开始日期**（bid_start_date）：招标公告发布或开始接受投标的日期
  - **招标结束日期**（bid_end_date）：招标截止日期
  - **中标日期**（award_date）：公布中标结果的日期（招标中的项目此字段为空）
